...
StartWeight:
    ...
    Loop,
    {
        ...
        If (ErrorLevel = 0) {
            ...
            Loop,
            {
                aa := A_TickCount - WeightTask
                StartTime22 := A_TickCount
                ImageSearch, xx, yy, 250, 220, 560, 440, *25 bin\yellow.png
                if (Errorlevel = 0) {
                    calxx:=xx+5
                    calyy:=yy+5
                    Click, %calxx% %calyy% 10
                    Sleep 70
                    MouseMove, 400, 541, 0
                    MouseMove, 400, 540, 0
                    Sleep 20
...
StartSS:
    ...
    IniWrite, %SSP%, settings.ini, StrikeSpeed, SSP
    IniWrite, %SST%, settings.ini, StrikeSpeed, SST
    IniWrite, %SSE%, settings.ini, StrikeSpeed, SSE
    IniWrite, %SSD%, settings.ini, StrikeSpeed, SSD
    ...
    {
        Gosub, Check
        ImageSearch,,, 65, 525, 750, 585, *Trans0x242424 bin/ss.bmp ;; SS picture
        If (ErrorLevel = 0) {
            ;; Waling path
            If (SST = "Karate") {
                If (SSP = "Custom") {
...
SetOptions:
    ...
    GuiControl, ChooseString, SSP, %SSP%
...
(
    {
        ...
        "embeds": [
            {
                "title": "STRIKE SPEED | TOTAL RUNS: 100!",
                "color": null,
                "fields": [
                    {
                        "name": "Time Taken",
...
;postdata={"username":"i love vivace's macro","embeds": [{"fields": [{"name": "Session","value": "#123","inline": true},{"name": "Time","value": "#29.22","inline": true}],"author": {"name": "Strike Speed Session A_TimeNow"}}]}
WinHttpReq.Send(cc)
